...
public class DmaUserEventIndexTest extends GuiceTestCase {
  @Test
  public void testAll() throws Exception {
    Provider<DmaUserEventIndex> dmaUserEventIndexProvider = getDmaUserEventIndexProvider();
    DmaUserEventIndex userEventIndex = dmaUserEventIndexProvider.get();
  ...
  public void testBlock() throws Exception {
    ...
    long pointer = 2000;
    int minId = 5;
    ByteBuffer metaDataByteBuffer = ByteBuffer.allocate(DmaUserEventIndex.Block.MetaData.SIZE);
    ByteBuffer blockByteBuffer = ByteBuffer.allocate(1000);
    long prevBlockPointer = 1000;
    long nextBlockPointer = 3000;

    DmaUserEventIndex.Block.MetaData metaData = new DmaUserEventIndex.Block.MetaData(metaDataByteBuffer);
    DmaUserEventIndex.Block block = new DmaUserEventIndex.Block(metaData, blockByteBuffer);
    ...
    block = new DmaUserEventIndex.Block(new DmaUserEventIndex.Block.MetaData(metaDataByteBuffer),
        blockByteBuffer);
    Assert.assertEquals(minId, block.getMetaData().getMinId());
    Assert.assertEquals(pointer, block.getMetaData().getPointer());
    Assert.assertEquals(prevBlockPointer, block.getMetaData().getPrevBlockPointer());
  ...
  public void testIndexEntry() throws Exception {
    ...
    DmaUserEventIndex.IndexEntry indexEntry = new DmaUserEventIndex.IndexEntry(
        new AtomicInteger(numRecords), minId, pointers, minIds);
    DmaUserEventIndex.IndexEntry.Schema schema = new DmaUserEventIndex.IndexEntry.Schema(
        numPointers);
    ...
    for (int i = 1; i < 5; i++) {
      DmaUserEventIndex.Block.MetaData metaData = new DmaUserEventIndex.Block.MetaData(
          ByteBuffer.allocate(DmaUserEventIndex.Block.MetaData.SIZE));
      metaData.setBlockOffset(i);
      metaData.setPointer(i * 100);
      metaData.setMinId(i * 10);
      indexEntry.shiftBlock(new DmaUserEventIndex.Block(metaData, null));
    }
  ...
  private Provider<DmaUserEventIndex> getDmaUserEventIndexProvider() {
    ...
    Injector injector = createInjectorFor(
        prop, new DmaUserEventIndexModule());
    return injector.getProvider(DmaUserEventIndex.class);
  }
}
