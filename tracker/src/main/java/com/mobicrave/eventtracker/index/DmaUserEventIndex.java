...
public class DmaUserEventIndex implements Closeable {
  ...
  public DmaUserEventIndex(String filename, DmaList<IndexEntry> index,
  ...
  public int getEventOffset(int userId, long eventId) {
    ...
    if (eventId >= minIdInIndex) { // all blocks are in index
      ...
      Block block = blockFactory.find(blockFactory.find(
          indexEntry.getPointer(numPointersPerIndexEntry - 1)).getMetaData().getNextBlockPointer());
      while (block != null) {
  ...
  public void addEvent(int userId, long eventId) {
    ...
    if (!userIdSet.get(userId)) {
      ...
      if (numRecords % numRecordsPerBlock == 0) { // need a new block
        ...
        Block prevBlock = findBlock(indexEntry, 0);
  ...
  public static class IndexEntry {
    ...
    public static class Schema implements com.mobicrave.eventtracker.base.Schema<IndexEntry> {
    ...
    public static class Factory {
      ...
      public IndexEntry build() {
        long[] pointers = new long[numPointers];
        long[] minIds = new long[numPointers];
        return new DmaUserEventIndex.IndexEntry(new AtomicInteger(0), -1, pointers, minIds);
      }
    }
  }
}
